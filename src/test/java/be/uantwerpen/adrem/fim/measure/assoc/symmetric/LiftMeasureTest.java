...
public class LiftMeasureTest extends RuleMeasureTestBase {
	...
	public void test_set_set_equal() {
		Assert.assertEquals(5.0 / 3.0, m.evaluate(s0, s0), delta);
		Assert.assertEquals(5.0 / 3.0, m.evaluate(s01, s01), delta);
		Assert.assertEquals(5.0 / 2.0, m.evaluate(s012, s012), delta);
		Assert.assertEquals(5.0 / 1.0, m.evaluate(s47, s47), delta);
		Assert.assertEquals(5.0 / 2.0, m.evaluate(s56, s56), delta);
	}

	@Override
	@Test
	public void test_set_set_closed() {
		Assert.assertEquals(5.0 / 3.0, m.evaluate(s0, s01), delta);
		Assert.assertEquals(5.0 / 3.0, m.evaluate(s01, s0), delta);
	}

	@Override
	@Test
	public void test_set_set_include() {
		Assert.assertEquals(5.0 / 3.0, m.evaluate(s0, s012), delta);
		Assert.assertEquals(5.0 / 3.0, m.evaluate(s01, s012), delta);

		Assert.assertEquals(5.0 / 3.0, m.evaluate(s012, s0), delta);
		Assert.assertEquals(5.0 / 3.0, m.evaluate(s012, s01), delta);
	}

	@Override
	@Test
	public void test_set_set_overlap() {
		Assert.assertEquals(5.0 / 6.0, m.evaluate(s0, s56), delta);

		Assert.assertEquals(5.0 / 6.0, m.evaluate(s01, s56), delta);

		Assert.assertEquals(5.0 / 6.0, m.evaluate(s56, s0), delta);
		Assert.assertEquals(5.0 / 6.0, m.evaluate(s56, s01), delta);
	}
