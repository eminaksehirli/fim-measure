...
public class YulesYMeasureTest extends RuleMeasureTestBase {

	private YulesYMeasure m;

	@Override
	@Before
	public void setUp() throws Exception {
		super.setUp();
		m = new YulesYMeasure(db);
	}
	...
	public void test_set_set_overlap() {
		Assert.assertEquals((1 - sqrt(2)) / (1 + sqrt(2.0)),
				m.evaluate(s0, s56), delta);

		Assert.assertEquals((1 - sqrt(2)) / (1 + sqrt(2.0)),
				m.evaluate(s01, s56), delta);

		Assert.assertEquals((1 - sqrt(2)) / (1 + sqrt(2.0)),
				m.evaluate(s56, s0), delta);
		Assert.assertEquals((1 - sqrt(2)) / (1 + sqrt(2.0)),
				m.evaluate(s56, s01), delta);
	}

	@Override
	@Test
	public void test_set_set_no_overlap() {
		Assert.assertEquals(-1.0, m.evaluate(s0, s47), delta);

		Assert.assertEquals(-1.0, m.evaluate(s01, s47), delta);

		Assert.assertEquals(-1.0, m.evaluate(s012, s47), delta);
		Assert.assertEquals(-1.0, m.evaluate(s012, s56), delta);

		Assert.assertEquals(-1.0, m.evaluate(s47, s0), delta);
		Assert.assertEquals(-1.0, m.evaluate(s47, s01), delta);
		Assert.assertEquals(-1.0, m.evaluate(s47, s012), delta);
		Assert.assertEquals(-1.0, m.evaluate(s47, s56), delta);

		Assert.assertEquals(-1.0, m.evaluate(s56, s012), delta);
		Assert.assertEquals(-1.0, m.evaluate(s56, s47), delta);
	}
}
