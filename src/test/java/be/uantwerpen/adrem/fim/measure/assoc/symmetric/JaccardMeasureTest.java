 ...
 */
public class JaccardMeasureTest extends RuleMeasureTestBase {

	private JaccardMeasure m;

	@Override
	@Before
	public void setUp() {
		super.setUp();
		m = new JaccardMeasure(db);
	}
	...
	public void test_set_set_include() {
		assertEquals(2.0 / 3.0, m.evaluate(s0, s012), delta);
		assertEquals(2.0 / 3.0, m.evaluate(s01, s012), delta);
	...
	public void test_set_set_overlap() {
		assertEquals(1.0 / 4.0, m.evaluate(s0, s56), delta);

		assertEquals(1.0 / 4.0, m.evaluate(s01, s56), delta);

		assertEquals(1.0 / 4.0, m.evaluate(s56, s0), delta);
		assertEquals(1.0 / 4.0, m.evaluate(s56, s01), delta);
	}
