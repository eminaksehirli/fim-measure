...
public class RelativeRiskMeasureTest extends RuleMeasureTestBase {
	...
	public void test_set_set_include() {
		Assert.assertEquals(POSITIVE_INFINITY, m.evaluate(s0, s012), delta);
		Assert.assertEquals(POSITIVE_INFINITY, m.evaluate(s01, s012), delta);

		Assert.assertEquals(3.0, m.evaluate(s012, s0), delta);
		Assert.assertEquals(3.0, m.evaluate(s012, s01), delta);
	}

	@Override
	@Test
	public void test_set_set_overlap() {
		Assert.assertEquals(2.0 / 3.0, m.evaluate(s0, s56), delta);

		Assert.assertEquals(2.0 / 3.0, m.evaluate(s01, s56), delta);

		Assert.assertEquals(3.0 / 4.0, m.evaluate(s56, s0), delta);
		Assert.assertEquals(3.0 / 4.0, m.evaluate(s56, s01), delta);
	}
