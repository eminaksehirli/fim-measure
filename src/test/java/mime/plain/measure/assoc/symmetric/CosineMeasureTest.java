...
public class CosineMeasureTest extends RuleMeasureTestBase {
	...
	public void test_empty_set() {
		Assert.assertEquals(sqrt(3.0 / 5.0), m.evaluate(sEmpty, s0), delta);
		Assert.assertEquals(sqrt(3.0 / 5.0), m.evaluate(sEmpty, s01), delta);
		Assert.assertEquals(sqrt(2.0 / 5.0), m.evaluate(sEmpty, s012), delta);
		Assert.assertEquals(sqrt(1.0 / 5.0), m.evaluate(sEmpty, s47), delta);
		Assert.assertEquals(sqrt(2.0 / 5.0), m.evaluate(sEmpty, s56), delta);
	}

	@Override
	@Test
	public void test_set_empty() {
		Assert.assertEquals(sqrt(3.0 / 5.0), m.evaluate(s0, sEmpty), delta);
		Assert.assertEquals(sqrt(3.0 / 5.0), m.evaluate(s01, sEmpty), delta);
		Assert.assertEquals(sqrt(2.0 / 5.0), m.evaluate(s012, sEmpty), delta);
		Assert.assertEquals(sqrt(1.0 / 5.0), m.evaluate(s47, sEmpty), delta);
		Assert.assertEquals(sqrt(2.0 / 5.0), m.evaluate(s56, sEmpty), delta);
	}
	...
	public void test_set_set_include() {
		Assert.assertEquals(2.0 / (5.0 * sqrt(6.0 / 25.0)),
				m.evaluate(s0, s012), delta);
		Assert.assertEquals(2.0 / (5.0 * sqrt(6.0 / 25.0)),
				m.evaluate(s01, s012), delta);

		Assert.assertEquals(2.0 / (5.0 * sqrt(6.0 / 25.0)),
				m.evaluate(s012, s0), delta);
		Assert.assertEquals(2.0 / (5.0 * sqrt(6.0 / 25.0)),
				m.evaluate(s012, s01), delta);
	}

	@Override
	@Test
	public void test_set_set_overlap() {
		Assert.assertEquals(1.0 / (5.0 * sqrt(6.0 / 25.0)),
