...
public class JaccardMeasureTest extends RuleMeasureTestBase {
	...
	public void test_set_set_overlap() {
		Assert.assertEquals(1.0 / 4.0, m.evaluate(s0, s56), delta);

		Assert.assertEquals(1.0 / 4.0, m.evaluate(s01, s56), delta);

		Assert.assertEquals(1.0 / 4.0, m.evaluate(s56, s0), delta);
		Assert.assertEquals(1.0 / 4.0, m.evaluate(s56, s01), delta);
	}
