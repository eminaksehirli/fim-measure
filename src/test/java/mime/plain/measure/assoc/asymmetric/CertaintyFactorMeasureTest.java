...
public class CertaintyFactorMeasureTest extends RuleMeasureTestBase {
	...
	private CertaintyFactorMeasure m;

	@Override
	@Before
	public void setUp() throws Exception {
		super.setUp();
		m = new CertaintyFactorMeasure(db);
	}
	...
	public void test_set_set_include() {
		Assert.assertEquals((2.0 / 3.0 - 2.0 / 5.0) / (1 - 2.0 / 5.0),
	...
	public void test_set_set_overlap() {
		Assert.assertEquals((1.0 / 3.0 - 2.0 / 5.0) / (1 - 2.0 / 5.0),
				m.evaluate(s0, s56), delta);

		Assert.assertEquals((1.0 / 3.0 - 2.0 / 5.0) / (1 - 2.0 / 5.0),
				m.evaluate(s01, s56), delta);

		Assert.assertEquals((1.0 / 2.0 - 3.0 / 5.0) / (1 - 3.0 / 5.0),
	...
	public void test_set_set_no_overlap() {
		Assert.assertEquals((-1.0 / 5.0) / (1 - 1.0 / 5.0),
				m.evaluate(s0, s47), delta);

		Assert.assertEquals((-1.0 / 5.0) / (1 - 1.0 / 5.0),
				m.evaluate(s01, s47), delta);

		Assert.assertEquals((-1.0 / 5.0) / (1 - 1.0 / 5.0),
				m.evaluate(s012, s47), delta);
		Assert.assertEquals((-2.0 / 5.0) / (1 - 2.0 / 5.0),
				m.evaluate(s012, s56), delta);

		Assert.assertEquals((-3.0 / 5.0) / (1 - 3.0 / 5.0),
				m.evaluate(s47, s0), delta);
		Assert.assertEquals((-3.0 / 5.0) / (1 - 3.0 / 5.0),
				m.evaluate(s47, s01), delta);
		Assert.assertEquals((-2.0 / 5.0) / (1 - 2.0 / 5.0),
				m.evaluate(s47, s012), delta);
		Assert.assertEquals((-2.0 / 5.0) / (1 - 2.0 / 5.0),
				m.evaluate(s47, s56), delta);

		Assert.assertEquals((-2.0 / 5.0) / (1 - 2.0 / 5.0),
				m.evaluate(s56, s012), delta);
		Assert.assertEquals((-1.0 / 5.0) / (1 - 1.0 / 5.0),
				m.evaluate(s56, s47), delta);
